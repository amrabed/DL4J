 ...
 * Given a text file and a few options, generate feature vectors and labels for
 * training, where we want to predict the next character in the sequence.<br>
 * This is done by randomly choosing a position in the text file to start the
 * sequence and (optionally) scanning backwards to a new line (to ensure we
 * don't start half way through a word for example).<br>
...
public class CharacterIterator implements DataSetIterator
{
	...
	int maxSize = lines.size(); // add lines.size() to account for newline
